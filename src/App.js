...
// --- Iconos y useWindowSize (con corrección en ChatsIcon) ---
// --- CORRECCIÓN: El atributo 'd' del path de ChatsIcon ha sido reemplazado por un SVG válido ---
...
function DashboardLayout({ currentUser }) {
    ...
    const handleLogout = async () => {
        ...
        // La redirección será manejada por el onAuthStateChange en App.js
    };
...
function App() {
    ...
    const syncUserProfile = useCallback(async () => {
        try {
            console.log("Sincronizando perfil de usuario...");
            const profile = await getCurrentUserProfile();
            setUserProfile(profile);
            localStorage.setItem('cachedUserProfile', JSON.stringify(profile));
            console.log("Perfil de usuario sincronizado:", profile);
        } catch (error) {
