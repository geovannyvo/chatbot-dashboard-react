...
const MoreOptionsIcon = ({ onClick }) => ( <svg onClick={onClick} viewBox="0 0 24 24" width="20px" height="20px" style={{ cursor: 'pointer', fill: colors.textSec, marginLeft: 'auto', flexShrink: 0 }} title="Más opciones"> <path d="M12 8c1.1 0 2-.9 2-2s-.9-2-2-2-2 .9-2 2 .9 2 2 2zm0 2c-1.1 0-2 .9-2 2s.9 2 2 2 2-.9 2-2-.9-2-2-2zm0 6c-1.1 0-2 .9-2 2s.9 2 2 2 2-.9 2-2-.9-2-2-2z"/> </svg> );
...
const UnreadBadge = React.memo(() => (
    <span style={{ backgroundColor: colors.accent, width: '10px', height: '10px', borderRadius: '50%', display: 'inline-block', marginLeft: 'auto', marginRight: '8px', flexShrink: 0 }} title="Mensajes no leídos"></span>
));
    ...
    const orderedSessionIds = useMemo(() => {
        return Object.keys(groupedInteractions)
            ...
            .filter(sessionId => {
                ...
                if (chatViewFilter === 'active') { return status !== SESSION_STATUS.ARCHIVED && status !== SESSION_STATUS.BLOCKED && status !== SESSION_STATUS.NEEDS_AGENT; } 
                else if (chatViewFilter === 'archived') { return status === SESSION_STATUS.ARCHIVED; } 
                else if (chatViewFilter === 'blocked') { return status === SESSION_STATUS.BLOCKED; } 
                else if (chatViewFilter === 'needs_agent') { return status === SESSION_STATUS.NEEDS_AGENT; }
    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', height: '100%', backgroundColor: colors.bgSec, borderRight: `1px solid ${colors.border}` }}>
            <div style={{ padding: '10px 15px', position: 'relative', flexShrink: 0, backgroundColor: colors.bgSec, borderBottom: `1px solid ${colors.border}`}}>
                <SearchIcon />
                <input type="text" placeholder="Buscar por nombre o número" value={searchTerm} onChange={(e) => onSearchChange(e.target.value)} style={{ width: '100%', boxSizing: 'border-box', padding: '8px 15px 8px 45px', borderRadius: '8px', border: 'none', backgroundColor: colors.bgMain, color: colors.textPrimary, fontSize: '0.9em', outline: 'none' }} />
            </div>
            
            {isLoading && orderedSessionIds.length === 0 ? (
                ...
                <ul style={{ listStyle: 'none', padding: 0, margin: 0, height: '100%', overflowY: 'auto' }}>
                    {orderedSessionIds.map(sessionId => {
                       const messages = groupedInteractions[sessionId] || []; const lastMessage = messages.length > 0 ? messages[messages.length - 1] : null; const sessionData = sessionStatuses[sessionId] || { status: SESSION_STATUS.BOT_ACTIVE, is_pinned: false }; const status = sessionData.status; const isSelected = sessionId === selectedSessionId; const unreadCount = unreadCounts[sessionId] || 0; const isBlocked = status === SESSION_STATUS.BLOCKED; const isPinned = sessionData.is_pinned; const showUnreadIndicator = unreadCount > 0 && !isBlocked; let itemStyle = { padding: '12px 15px', borderBottom: `1px solid ${colors.border}`, cursor: 'pointer', backgroundColor: isSelected ? colors.bgSelected : colors.bgMain, transition: 'background-color 0.1s ease-out', opacity: (isBlocked && chatViewFilter !== 'blocked') ? 0.6 : 1, display: 'flex', flexDirection: 'column', position: 'relative' }; if (status === SESSION_STATUS.NEEDS_AGENT && !isSelected) { itemStyle.backgroundColor = colors.bgWarning; itemStyle.borderLeft = `3px solid ${colors.btnYellow}`; } let statusIndicator = null; if (isBlocked) { statusIndicator = <BlockIconMini />; } else if (status === SESSION_STATUS.NEEDS_AGENT) { statusIndicator = <AlertIconMini />; } else if (status === SESSION_STATUS.AGENT_ACTIVE) { statusIndicator = <AgentIconMini />; } else if (status === SESSION_STATUS.ARCHIVED) { statusIndicator = <ArchiveIconMini />; } else { statusIndicator = <BotIconMini />; } const handleMouseEnter = (e) => { if (!isSelected) e.currentTarget.style.backgroundColor = colors.bgHover; }; const handleMouseLeave = (e) => { if (!isSelected) { e.currentTarget.style.backgroundColor = (status === SESSION_STATUS.NEEDS_AGENT) ? colors.bgWarning : colors.bgMain; } }; const lastMessageContent = lastMessage ? ( lastMessage.isAgentMessage ? `Agente: ${lastMessage.content}` : lastMessage.type === 'human' ? `Usuario: ${lastMessage.content}` : `Bot: ${lastMessage.content}` ) : 'Sin mensajes aún.'; const lastMessageTimeFormatted = lastMessage ? new Date(lastMessage.time).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : '';
                        ...
                        return (
                            <li key={sessionId} style={itemStyle} onClick={() => onSelectSession(sessionId)} onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave} >
                                <div style={{ display: 'flex', alignItems: 'center', marginBottom: '5px' }}>
                                    {(isPinned && (chatViewFilter === 'active' || chatViewFilter === 'needs_agent')) && <PinIcon />}
                                    {statusIndicator}
                                    
                                    <span style={{ fontWeight: '600', fontSize: '0.9em', color: colors.textPrimary, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', flexGrow: 1, paddingRight: '5px', textDecoration: isBlocked ? 'line-through' : 'none' }} title={displayName}>
                                        {displayName}
                                    </span>
                                ...
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingLeft: '21px' }}>
                                    <p style={{ fontSize: '0.85em', color: colors.textSec, margin: 0, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', fontWeight: showUnreadIndicator ? 'bold' : 'normal', flexGrow: 1, paddingRight: '10px', textDecoration: isBlocked ? 'line-through' : 'none', }} title={lastMessage?.content}>
                                        {lastMessageContent}
                                    </p>
                                    {showUnreadIndicator && <UnreadBadge />}
                                    {lastMessage && (
                                        <small style={{ fontSize: '0.75em', marginLeft: showUnreadIndicator ? '0px' : '5px', color: showUnreadIndicator ? colors.accent : colors.textSec, fontWeight: showUnreadIndicator ? 'bold' : 'normal', flexShrink: 0, textDecoration: isBlocked ? 'line-through' : 'none', }}>
